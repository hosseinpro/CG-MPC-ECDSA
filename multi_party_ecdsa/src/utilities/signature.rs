...
use crate::utilities::class_group::scalar_to_mpz;
...
impl Signature {
    pub fn verify(&self, pubkey: &ProjectivePoint, message: &Scalar) -> Result<(), MulEcdsaError> {
        ...
        let u1_plus_u2_x = Mpz::from_bytes_be(MpzSign::Plus, x_bytes);

        let r_mpz = scalar_to_mpz(&self.r);
        let s_mpz = scalar_to_mpz(&self.s);
        
        // second condition is against malleability
        if r_mpz == u1_plus_u2_x && s_mpz < &q - &s_mpz
        {
            Ok(())
        } else {
