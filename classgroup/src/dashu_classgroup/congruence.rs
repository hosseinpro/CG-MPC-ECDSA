...
///
/// It is allowed (but inefficient) to generate a fresh `CongruenceContext`
/// for each call to `solve_linear_congruence`.
///
/// `self.solve_linear_congruence` can be called no matter what values
/// this struct's public members hold, so long as they are valid `BigInt` values.
...
impl CongruenceContext {
    ...
    pub fn solve_linear_congruence(
        ...
        ffi::bigint_gcdext(&mut self.g, &mut self.d, mu, a, m);
        if cfg!(test) {
            println!(
                "g = {}, d = {}, e = {}, a = {}, m = {}",
        ...
        if cfg!(debug_assertions) {
            ffi::bigint_fdiv_qr(&mut self.q, &mut self.r, b, &self.g);
            ...
            ffi::bigint_divexact(&mut self.q, b, &self.g)
        }
        ffi::bigint_mul(&mut self.r, &self.q, &self.d);
        ffi::bigint_tdiv_r(mu, &self.r, m);
        if let Some(v) = v {
            if cfg!(debug_assertions) {
                ffi::bigint_fdiv_qr(v, &mut self.r, &m, &self.g);
                debug_assert!(self.r.is_zero(), "Could not solve the congruence ― did you pass a non-prime or a positive number to the command line tool?!");
            } else {
                ffi::bigint_divexact(v, &m, &self.g)
            }
        }
    }
}
